...
garden_harvest %>%
  ...
  summarize(total_weight_in_lbs = sum(weight * 0.00220462)) %>% 
  pivot_wider(names_from = day_of_week,
              values_from = total_weight_in_lbs)
...
garden_harvest %>% 
  ...
  summarize(total_harvest_lbs = sum(weight)) %>%
  left_join(plant_date_loc, 
            by = "variety")
```

Plot does show up on my table but I am not sure if it is correct. We also have alot of missing    
...
garden_harvest %>% 
  ...
  arrange(date) %>% 
  mutate(total_lbs_harvest = sum(weight)) %>% 
  ggplot() +
  geom_bar(aes(y=fct_reorder(variety, total_lbs_harvest)))
```
